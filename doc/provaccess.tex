...
\begin{verbatim}
{provdal-base-url}?ID=rave:dr4&STEP=LAST&FORMAT=PROV-JSON
\end{verbatim}
...
\begin{table}[h]
\small
\begin{tabulary}{1.0\textwidth}{@{}lLp{0.2\textwidth}p{0.3\textwidth}@{}}
%{llp{0.2\textwidth}p{0.3\textwidth}}
\toprule
\head{Parameter} & \head{Requirement} & \head{Value/options} & \head{Description}\\\hline
\midrule
ID & required & qualified ID & a valid qualified identifier for an entity or activity (can occur multiple times)\\
STEP & optional & LAST, ALL & \texttt{LAST} for retrieving only the last provenance step or \texttt{ALL} for everything\\
