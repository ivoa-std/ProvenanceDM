...
invalidatedAtTime  & datetime & date and time of the destruction or cessation of the entity. The entity is no longer available for use (or further invalidation) after invalidation.\\
...
%If a \class{Used}/\class{WasGeneratedBy} object is linked to a corresponding description element, then there must exist a link between the related activity and its corresponding activity description.
...
default     & string & a reference value defined in the context of a project for the corresponding value entities as a string whose value can be interpreted by the \attribute{valueType} attribute \\
\bottomrule
\end{tabulary}
