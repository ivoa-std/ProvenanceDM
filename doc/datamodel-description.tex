...
start with an general UML class diagram as an overview (Section~\ref{sec:overview}) and explain the core elements with details for each class and relation  (Section~\ref{sec:core_model}). We then define the extended model, introducing and defining specialised concepts and relation that brings useful provenance information for astronomy (Section~\ref{sec:extended_model}).
...
that were used in the Provenance DM of the World Wide Web Consortium
(W3C PROV, \citealt{std:W3CProvDM}), which defines a very abstract pattern that can
be reused here. The provenance information resides in the relations between those core elements, also taken from the W3C PROV-DM and ontology.

In the domain of astronomy, certain processes and steps are repeated over and
over again, maybe using a different configuration and within a different context. 
We therefore separate the descriptions
of activities from the actual processes and introduce an additional
\class{ActivityDescription} class (see
Figure~\ref{fig:classdiagram-conceptional}). Likewise, we also apply the same
pattern for \class{Entity} and add an \class{EntityDescription} class.
Defining such descriptions allows them to be reused, which is less redundant 
when performing a series of tasks of the same type. 

A similar normalization of descriptions of the actual processes and datasets 
can be found in the IVOA Simulation DM \citep[SimDM, ][]{std:SimDM}),
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.6]{classes-core-w3c.pdf}
...
Figure~\ref{fig:coreclasses}. 
Here are the core classes, similar to the W3C PROV-DM core classes, with short descriptions and some examples:
...
\item \class{Agent:} executes\slash{}controls an activity, is responsible for
    an activity or an entity. For example: telescope astronomer,
...
We use the following relation classes to specify
the mapping between the three core  classes. 
The relation names were, again, chosen to match the W3C PROV-DM names:
\begin{itemize}
...
The W3C PROV-DM and ontology contain several components that are not described in this document. The IVOA Provenance DM having the same core concepts, it is possible to extend it with any W3C PROV component or extension.
...
\toprule
\head{Attribute} & \head{W3C PROV-DM} & \head{Data type} & \head{Description}\\
\midrule
...
\bottomrule
\end{tabulary}
\caption[Attributes of \class{Entities}]{Attributes of \class{Entities}. Mandatory attributes are marked in \textbf{bold}, references are indicated with an arrow ($\rightarrow$). Further project-specific attributes (e.g. size, path, url, \dots) could be addedwhen relevant for the project (see Section~\ref{sec:dataset-obscore}). Attributes from \class{EntityDescription} (see next section) may appear here as well.
...
this relation, linking two entities
together, is also borrowed from the W3C model. It is used to express that one
entity was derived from another, e.g. it can be used to find one (or more)
progenitor(s) of a dataset.
...
descriptions. Nevertheless, we include \class{WasDerivedFrom} for those cases
...
inferred from following \class{WasGeneratedBy} and \class{Used} relations alone:
...
\class{Collection} class via a composition. It may contain an additional role attribute, or an order inside the collection.
...
\bottomrule
\end{tabulary}
\caption[Attributes of \class{Activity}.]{Attributes of \class{Activity}, their data types and equivalents in the W3C PROV-DM, if existing. Attributes in bold are \textbf{mandatory}, references are indicated with an arrow ($\rightarrow$).}
\end{table}
...
\bottomrule
\end{tabulary}
\caption[Attributes of the \class{WasInformedBy} relation]{Attributes of the
\class{WasInformedBy} relation. The attribute names correspond to the W3C PROV-DM names.
...
%\begin{figure}[ht]
%\centering
%\includegraphics[scale=0.6]{entity-activity-relations.pdf}
%\hspace{0.15\textwidth}
%\includegraphics[scale=0.6]{entity-activity-relations-nodesc.pdf}
%\caption[\class{Entity}-\class{Activity} relations]{\class{Entity} and \class{Activity} are linked via the \class{Used} and \class{WasGeneratedBy} relations. In the left image, the \attribute{role} that an entity played when being used or generated by an activity is recorded with the corresponding \emph{UsedDescription} and \emph{WasGeneratedByDescription}, also see Section~\ref{sec:entity-roles}. If these description classes are not used, the \emph{role} can be used directly as an attribute within the \class{Used} and \class{WasGeneratedBy} classes (right image).}
...
\toprule
\head{Class or type} & \head{W3C PROV-DM} & \head{DatasetDM} &\head{Comment} \\
\midrule
Agent       & Agent  & Party & \\
...
For each agent a \attribute{name} must be specified. a summary of the attributes for \class{Agent} is given in Table~\ref{tab:agent-attributes}.
...
It would also increase the value of the given information if the (current) affiliation of the agent (and a project leader/group leader) were specified in order to maximise the chance of finding any contact person later on. 
...
There can also be more than one agent for each activity/entity with different \attribute{roles} and one agent can be responsible for more than one activity or entity. This many-to-many relationship is made explicit in our model by adding the two
following relation classes:

\begin{itemize}
\item \class{wasAssociatedWith}: relates an \class{Activity} to an agent
\item \class{wasAttributedTo}: relates an \class{Entity} to an agent
\end{itemize}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.0\textwidth]{classes-overview.pdf}
\caption[Detailed UML class diagram, compatible with VO-DML]{More detailed
overview of the classes for the Provenance DM. Note that this UML class
diagram is %  [M.N.: removed "more" -- "more" here would actually contradict the
           %         "fully VO-DML compliant" assertion of the main text...]
compatible with VO-DML.}
\label{fig:classdiagram}
\end{figure}

Figure~\ref{fig:classdiagram} shows the full class diagram. In addition to the core model (Section~\ref{sec:core_model}), we define specialised entities (Section\ref{sec:spec_entities}) and relations (Section\ref{sec:spec_relations}) that were identified as useful in one or several use cases. 
...
\subsubsection{Specialised Entities}
\label{sec:spec_entities}

The abstraction level of the W3C PROV-DM being high, one of the objective of this IVOA recommendation is to guide the usage of this model in the astronomy context by providing specialised entities that are connected to concepts known in astronomy and relevant to assess the quality and reliability of the exchanged entities.
...
We already expressed the necessity of adding \textbf{Descriptions} to the concepts of Activity and Entity, in order to avoid redundancy and give detail explanations on the method or algorithms that compose the core of an activity. In addition, from our use cases in Astronomy, activities require specific metadata that is related to the \textbf{Configuration} of an activity, the \textbf{Context}. We therefore define  specialised entities hierarchically following those main categories. The list given below is not intended to be exhaustive, additional project-dependant entities may be defined when relevant.

It is important to note that specialised entities inherit from the \class{Entity}, all those classes thus share the identifier attribute  \attribute{id}, as well as the relations of the core model. The \attribute{id} attribute must be unique for each different entity so that it can be connected to other concepts using core or specialised relations (see Section~\ref{sec:spec_relations}). It is possible to attach an Agent to any of those subclasses of \class{Entity} in order to provide specific contacts for the content of a specialised entity.

\paragraph{MainEntity}
Provenance information are expected to be recorded for those main entities, to which we may attach descriptions, detail the configuration that led to their generation and the context in which they were generated:
\begin{itemize}
...
\item[\textbf{ConfigFile}]: file containing configuration information for the activity, e.g. a list of parameters for a script, generally in a given format (txt, json, yaml...)
\item[\textbf{ObsConfig}]: structured list of parameters that gives the configuration of an observation.
\end{itemize}
...
\item[\textbf{InstrumentLayout}]: description of how the instrument is structured and how the hardware is set up before an activity starts.
...
\subsubsection{Specialised Relations}
\label{sec:spec_relations}

In order to distinguish the different specialised entities in their usage or influence, we define the corresponding specialised relations: \class{hadDescription}, \class{hadConfiguration} and \class{hadContext}, that should be connected to the entity categories \class{Description}, \class{Configuration} and \class{Context}, respectively.

We thus provide a way to qualify the usage or influence of a specialised entity on an activity, or another entity. The \attribute{role} attribute in those relations is not expected to be defined as they are already qualified. Those relation can thus be seen as simple association tables.
...
The \class{EntityDescription} class does \emph{not} contain any information about the
...
\textbf{value}   & & (value dependent) & the value of the parameter, type depends on ParameterDescription.datatype and xtype; follows same rules as VOTable TABLEDATA and DALI\\ % TODO: check and put proper reference somewhere!
\midrule
...
%The \class{Parameter} class is an entity that should be connected to an activity with the specialised usage relation hadConfiguration. 
The optional attributes of \class{ParameterDescription} are taken from the FIELD and PARAM elements in VOTable \citep{std:VOTABLE}. The \attribute{value} attribute should provide the default value proposed for a parameter.
...
\bottomrule
\end{tabulary}
\caption[Attributes of \class{ActivityDescription}]{Attributes of \class{ActivityDescription}.}
\end{table}
...
In order to describe more largely an activity, it is common to define the expected inputs and outputs of this activity. In the case of workflow descriptions, such as ProvONE (but also Kepler or Taverna), input and output \textbf{ports} are defined, and can be connected to build a workflow of activities. The description of workflows is out of the scope of this document, and the related concept we introduce here are the UsedDescription and the WasGeneratedByDescription classes. Those classes are meant to store
descriptive information about the usage or generation of an entity that is known before the activity is executed. 

In particular, if the \attribute{role} attribute is given in those description classes, the corresponding \class{Used} and \class{WasGeneratedBy} relations must contain the same role value. The \attribute{multiplicity} attributes can indicate that more than one entity will have the same role, e.g. in the case of the stacking of several images, all the input images share the exact same role.
...
multiplicity & string & Number of expected input entities to be used with the given role. The multiplicity is 1 by default and a * indicates an unlimited number of input entities.\\
\midrule
...
multiplicity & string & Number of expected output entities that will be generated with the given role. The multiplicity is 1 by default and a * indicates an unlimited number of input entities.\\
\midrule
...
\class{ParameterDescription} can be seen as a specialised input port attached to an \class{ActivityDescription}. We thus expect a list of parameters for an activity, described with the attributes listed in Section~\ref{sec:parameters}. This input port is not expected to have a \attribute{role} attribute, however the \attribute{name} attribute of a parameter generally indicates its role. Such a list of input parameters already exist in the IVOA ecosystem for service resources, in this context, they are written in the form of an IVOA DataLink Service Descriptor \citep{std:Datalink}.
