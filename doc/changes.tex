...
\item Change \attribute{doculink} attribute name to \attribute{docurl}.
\item Modify definition of \attribute{valueType} (VOTable-like attributes datatype, arraysize, xtype).
\item Remove \attribute{min}, \attribute{max}, \attribute{options}, \attribute{default} from \class{ValueDescription}.
\item \attribute{options} is an array of strings.
\end{itemize}
...
\item \class{WasInformedBy} and \class{WasDerivedFrom} are included in section \ref{sec:entity-activity-relations} on \class{Entity}-\class{Activity} relations. They are simple relations.
...
\item Grouping of the \class{Parameter}-\class{ParameterDescription} classes as a \class{ActivityConfiguration} package that also includes config files.
