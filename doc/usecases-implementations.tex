...
One may only use the core data model without the description classes if they are not needed for the project. In that case, it is recommended to merge the attributes of the description classes into the main classes (\class{Entity}/\class{Activity}/\class{Agent}, and if needed \class{Parameter}). In the same way, when serializing the provenance information, the description classes can be merged to the main classes, which is needed to produce W3C compliant provenance files.
...
It is also required at this level to disentangle \class{Entity} properties and \class{EntityDescription} properties: everything that you may know about an entity before its creation, belongs to the \class{EntityDescription} (e.g. file \attribute{format}, \attribute{content\_type}, \attribute{dataproduct\_type}, \attribute{category}, \dots).

\paragraph{Group common features for entities and/or activities}
If inside the project, the different entities that will be manipulated are already defined precisely, as well as the activities producing them, then the description classes are probably of interest and will help reducing
...
If description classes are relevant for a project, it may be sufficient to store the descriptions directly as \class{ActivityDescription} files (see Section \ref{sec:description-serialization}). 
...
\url{http://wiki.ivoa.net/twiki/bin/view/IVOA/ProvSerialisationExample/HiPS-prov-provn.txt}, the corresponding VOTable-format is available at \\
\url{http://wiki.ivoa.net/twiki/bin/view/IVOA/ProvSerialisationExample/HiPS-prov-vot.xml}.
